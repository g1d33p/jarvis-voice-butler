...
async def _bring_page_window_to_front(page: Page) -> None:
    """Best-effort: bring Yaadhamma's browser tab to the front.
...
# data-yaadhamma-id attribute, so it can be clicked later by id even when its
# visible text is split across many child elements (e.g. WhatsApp chat rows).
_INSPECT_SCRIPT = r"""elements => {
  document.querySelectorAll('[data-yaadhamma-id]')
    .forEach(el => el.removeAttribute('data-yaadhamma-id'));
  ...
  for (const el of elements) {
    ...
    el.setAttribute('data-yaadhamma-id', String(id));
    results.push({
      id: '#' + id,
...
class BrowserManager:
    ...
    async def snapshot(self) -> dict[str, object] | None:
        ...
        Returns None when Yaadhamma's browser is not running. Used by the
    ...
    async def close_browser(self, *, confirmed: bool = False) -> dict[str, object]:
        """Close Yaadhamma's whole browser window, with every tab in it.
    ...
    async def element_label(self, target: str) -> str:
        ...
        if not match:
            return target
        page = await self._get_page()
        locator = page.locator(f'[data-yaadhamma-id="{match.group(1)}"]')
        try:
