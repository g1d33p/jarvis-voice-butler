...
def test_diff_reports_app_and_tab_changes() -> None:
    ...
    assert "Front app changed from Finder to Sureedu's browser." in changes
...
def test_diff_reports_browser_open_and_close() -> None:
    ...
    assert diff(_obs(), open_browser) == ["Sureedu's browser was opened."]
    assert diff(open_browser, _obs()) == ["Sureedu's browser was closed."]
...
async def test_observe_survives_missing_permission() -> None:
    ...
    assert obs.to_dict()["sureedu_browser"] == "not open"
